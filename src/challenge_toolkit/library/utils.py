...
class Utils:
    ...
    def get_challenge_dir(category: str, slug: str) -> Path:
        return (
            ...
            .joinpath(Utils.slugify(category) or "")
            .joinpath(slug)
        )
    ...
    def validate_length(text, min_length, max_length, identifier):
        ...
        if len(text) < min_length:
            print(
                f"{identifier.capitalize()} must be at least {min_length} characters long."
            )
            return False

        if len(text) > max_length:
            print(
                f"{identifier.capitalize()} cannot be longer than {max_length} characters."
            )
            return False
