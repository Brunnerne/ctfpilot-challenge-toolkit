    ...
    def __init__(self, parent_parser=None):
        if parent_parser:
            ...
            self.parser = parent_parser.add_parser(
                "pipeline", help="Pipeline for CTF challenges"
            )
        else:
            self.parser = argparse.ArgumentParser(
                description="Pipeline for CTF challenges"
            )
        ...
        self.parser.add_argument(
            "registry", help="Registry to push the Docker image to"
        )
        self.parser.add_argument("image_prefix", help="Prefix for the Docker image")
        self.parser.add_argument(
            "--image_suffix", help="Suffix for the Docker image", default=""
        )
...
class Docker:
    ...
    def build(
        ...
        if (
            dockerfile_location.identifier
            and dockerfile_location.identifier.lower() not in ["none", "null", ""]
        ):
        ...
        try:
            build_command = [
                ...
                f"{image_full}:latest",
                "-t",
                f"{image_full}:{challenge.get_version()}",
                "-f",
                f"{Utils.get_challenge_dir(challenge.category, challenge.slug)}/{dockerfile_location.location}",
            ...
            if build_proc.returncode != 0:
                raise subprocess.CalledProcessError(
                    build_proc.returncode, build_command
                )

            push_command = ["docker", "push", image_full, "--all-tags"]
...
class DockerBuild:
    ...
    def run(self):
        ...
        for dockerfile_location in challenge.dockerfile_locations:
            print(
                f"Building Docker image for {dockerfile_location.identifier or 'default'}..."
            )
            Docker.build(
                docker.registry,
                docker.image_prefix,
